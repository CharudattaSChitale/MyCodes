...
public class SetBenchmark implements Benchmark {
	...
	int m_ops = 1;
	int nb_threads = 1;
	final static int MAX_OPERATIONS_PER_TRANSACTION = 20;
	...
	public void init(String[] args, int nb_threads) {
		...
		if (args.length > 0) {
			if (args[0].equals("OptimisticBoostedHashSet"))
				m_set = new OptimisticBoostedHashSet(10);
			else if (args[0].equals("LockFreeHashSet"))
				m_set=new LockFreeHashSet(10);
			else if (args[0].equals("OptimisticBoostedBucketList"))
				m_set=new OptimisticBoostedBucketList();
